...
//        $link = lang('sellers_url') . 'orders/';
    ...
    function paymentUncompleted( $order_code , $uid, $buyer ){
        ...
        $msg .= '<tr> <td style="padding: 10px;"> <table width="100%" cellpadding="0" cellspacing="0"> <tr> <td class="content-block aligncenter"> <table class="invoice"> <tr> <td> Dear '. $buyer['name'] .', <br><div> Thank you for shopping with us. Unfortunately, we were unable to process your payment for order <b>\'. $order_code.\'</b>.<br>This could have happened because: <ul style="margin-left:-10px;"> <li> You attempted to pay via international Debit/Credit card but we only accept testing cards. </li><li> You attempted to pay via Debit/Credit Card but wrongly introduced the PIN code or your card is not profiled for online payment. </li><li> You have not registered/activated your bank\'s 3D Secure system. </li><li> You have insufficient funds. </li></ul> If any amount was debited from your mobile money/bank account, please fill our Contact Us form or call our customer representatives for the refund. </div> </td></tr>';
        // Main product list
        $loop = ''; $x = 1; $subtotal = $shipping = 0;
        ...
        foreach( $orders->result() as $order ){
            ...
            $loop .= '<td class="aligncenter price">&#8358; ' .$order->amount . '</td>';
            $loop .= '<td class="aligncenter">' . $order->qty . '</td>';
        ...
        return $this->do_email($msg, 'Your Order - '. $order_code .' payment was unsuccessful completed.', $buyer['email'], 'sales@onitshamarket.com');
