...
Class Product_model extends CI_Model{
    ...
    function get_product( $id = ''){
        ...
        LEFT JOIN (SELECT var.product_id, SUM(var.qty) quantity FROM orders var WHERE var.product_id = {$id} AND var.payment_made = 'success') 
...
//                }
    ...
    function get_cart_details( $id ){
        $select = "SELECT p.product_status,p.weight, p.seller_id, u.first_name first_name,s.store_name, u.is_seller, i.image_name image FROM products p
...
//        $search = preg_replace("/[^a-z0-9]/", ' ', $search);
        ...
        $select .= "GROUP BY p.id ORDER BY relevance DESC LIMIT 5";
    ...
    function search_query_categories( $search ){
        ...
        INNER JOIN categories c ON(c.id = p.category_id) WHERE p.product_name LIKE '%" . $search . "%' ";
        $keys = explode(" ", $search );
        foreach ( $keys as $k ){
            $select .= " OR p.product_name LIKE '%{$k}%'";
        }
        $select .= "AND p.product_status = 'approved' GROUP BY p.category_id LIMIT 5";
        return $this->db->query( $select )->result();
    }

    //  Quick view query
    function get_quick_view_details( $id ){
        $select = "SELECT highlights,product_description,seller_id FROM products WHERE id = $id";
        return $this->db->query( $select)->row();
    }
